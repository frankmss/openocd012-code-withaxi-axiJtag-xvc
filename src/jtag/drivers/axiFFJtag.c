...
static void axi_ffjtag_opr_queue(affJtag_pkg *affjpkg) {
  ...
  while (1) {                               // main loop
    ...
    if ((alread_RBn >= needBereadBytes) &&
        (alread_SBn >= bytesLeft) &&
        ((ptr->full_offset & 0x0f00) == 0x0f00)) {  // if rec num > bytesLeft ,return;
...
static int axi_ffjtag_execute_queue(void) {
...
// affJtag swd operate!!!!
static int axi_ffjtag_swd_sequence(const uint8_t *seq, size_t length) {
  size_t left, write;
  uint32_t send;
  int err;
  ...
  while (left) {
    ...
    err = axi_ffjtag_transact(write, send, 0, NULL, NOT_REAL_SEND);
    if (err != ERROR_OK) return err;
    left -= write;
    seq += sizeof(uint32_t);
  };
...
static int axi_ffjtag_swd_switch_seq(enum swd_special_seq seq) {
  switch (seq) {
    case LINE_RESET:
      ...
      return axi_ffjtag_swd_sequence(swd_seq_line_reset,
                                       swd_seq_line_reset_len);
    case JTAG_TO_SWD:
      LOG_DEBUG_IO("JTAG-to-SWD");
      return axi_ffjtag_swd_sequence(swd_seq_jtag_to_swd,
                                       swd_seq_jtag_to_swd_len);
    case SWD_TO_JTAG:
      LOG_DEBUG_IO("SWD-to-JTAG");
      return axi_ffjtag_swd_sequence(swd_seq_swd_to_jtag,
...
static void axi_ffjtag_swd_read_reg(uint8_t cmd, uint32_t *value,
  ...
  /* cmd + ack */
  err = axi_ffjtag_transact(12, cmd, 0, &res, NOT_REAL_SEND);
  if (err != ERROR_OK) goto err_out;

  //ack = MASK_ACK(res);

  /* read data */
  err = axi_ffjtag_transact(32, 0, 0, &res, NOT_REAL_SEND);
  if (err != ERROR_OK) goto err_out;

  /* parity + trn */
  err = axi_ffjtag_transact(2, 0, 0, &rpar, NOT_REAL_SEND);
  if (err != ERROR_OK) goto err_out;
  axi_ffjtag_transact(0,0,0,0, REAL_SEND);
  ...
  switch (ack) {
    case SWD_ACK_OK:
      ...
      if (cmd & SWD_CMD_APNDP)
        err = axi_ffjtag_transact(ap_delay_clk, 0, 0, NULL,REAL_SEND);
        axi_ffjtag_clear_pkg(&affJtagPkg);
      queued_retval = err;
      return;
    case SWD_ACK_WAIT:
...
static void axi_ffjtag_swd_write_reg(uint8_t cmd, uint32_t value,
  ...
  cmd |= SWD_CMD_START | SWD_CMD_PARK;
  /* cmd + trn + ack */
  err = axi_ffjtag_transact(13, cmd, 0, &res, NOT_REAL_SEND);
  if (err != ERROR_OK) goto err_out;

  //ack = MASK_ACK(res);

  /* write data */
  err = axi_ffjtag_transact(32, value, 0, NULL, NOT_REAL_SEND);
  if (err != ERROR_OK) goto err_out;

  /* parity + trn */
  err = axi_ffjtag_transact(2, parity_u32(value), 0, NULL, NOT_REAL_SEND);
  if (err != ERROR_OK) goto err_out;
  axi_ffjtag_transact(0,0,0,NULL, REAL_SEND);
  ack = MASK_ACK(affJtagPkg.tdo[0].u32);
  axi_ffjtag_clear_pkg(&affJtagPkg);
...
static int axi_ffjtag_swd_run_queue(void) {
  ...
  err = axi_ffjtag_transact(8, 0, 0, NULL, REAL_SEND);
  axi_ffjtag_clear_pkg(&affJtagPkg);
  if (err != ERROR_OK) return err;
