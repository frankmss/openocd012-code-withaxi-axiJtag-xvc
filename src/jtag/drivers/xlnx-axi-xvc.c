...
static int xlnx_axi_xvc_read_reg(const int offset, uint32_t *val) {
  // uint32_t *b = (uint32_t *)(xlnx_axi_xvc->baseaddr + offset);
  ...
  uint32_t tmpVal = 0;
  switch (offset) {
    case XLNX_XVC_LEN_REG:
      tmpVal = xlnx_axi_xvc->ptr->length_offset;
      break;
    case XLNX_XVC_TMS_REG:
      tmpVal = xlnx_axi_xvc->ptr->tms_offset;
      break;
    case XLNX_XVC_TDI_REG:
      tmpVal = xlnx_axi_xvc->ptr->tdi_offset;
      break;
    case XLNX_XVC_TDO_REG:
      tmpVal = xlnx_axi_xvc->ptr->tdo_offset;
      break;
    case XLNX_XVC_CTRL_REG:
      tmpVal = xlnx_axi_xvc->ptr->ctrl_offset;
      break;
    default:
      LOG_DEBUG_IO("unsued reg : 0x%x\n", offset);
      break;
  }
  // uint32_t *w = xlnx_axi_xvc->baseaddr + offset;
  if (val) {
    *val = tmpVal;
    if (offset == XLNX_XVC_CTRL_REG) {
      // LOG_DEBUG_IO("ctl reg : 0x%x\n", *val);
    }
  }
...
static int xlnx_axi_xvc_write_reg(const int offset, const uint32_t val) {
  ...
  switch (offset) {
    case XLNX_XVC_LEN_REG:
      (xlnx_axi_xvc->ptr->length_offset) = val;
      break;
    case XLNX_XVC_TMS_REG:
      (xlnx_axi_xvc->ptr->tms_offset) = val;
      break;
    case XLNX_XVC_TDI_REG:
      (xlnx_axi_xvc->ptr->tdi_offset) = val;
      break;
    case XLNX_XVC_TDO_REG:
      (xlnx_axi_xvc->ptr->tdo_offset) = val;
      break;
    case XLNX_XVC_CTRL_REG:
      (xlnx_axi_xvc->ptr->ctrl_offset) = val;
      break;
    default:
      LOG_DEBUG_IO("unsued reg : 0x%x\n", offset);
      break;
...
static int xlnx_axi_xvc_transact(size_t num_bits, uint32_t tms, uint32_t tdi,
  ...
  err =
      xlnx_axi_xvc_write_reg(XLNX_XVC_CTRL_REG, XLNX_XVC_CTRL_REG_ENABLE_MASK);
  if (err != ERROR_OK) return err;

  while (!done) {
    usleep(500);  // important ,there is a bug in the axiJtag ip cahill report
    err = xlnx_axi_xvc_read_reg(XLNX_XVC_CTRL_REG, &ctrl);
    if (err != ERROR_OK) return err;

    if (!(ctrl & XLNX_XVC_CTRL_REG_ENABLE_MASK)) done = 1;
  ...
  if (tdo)
    LOG_DEBUG_IO("Transact num_bits: %zu, tms: %x, tdi: %x, tdo: %x", num_bits,
                 tms, tdi, *tdo);
  else
...
static int xlnx_axi_xvc_execute_stableclocks(struct jtag_command *cmd) {
...
static int xlnx_axi_xvc_execute_scan(struct jtag_command *cmd) {
  ...
  while (left) {
    ...
    err =
        xlnx_axi_xvc_transact(write, tms, tdi, type != SCAN_OUT ? &tdo : NULL);
...
// cahill add from usb_blaster
static void xlnx_initial_wipeout(void) {
  // uint32_t val;
  // xlnx_axi_xvc_transact(16, 0x00, 0, &val);
  // xlnx_axi_xvc_transact(16, 0xff, 0, &val);
  // xlnx_axi_xvc_transact(16, 0x00, 0, &val);
}
...
// cahill add ctrl_offset[1] to reset axiJtagIp,
// elsewise, the ip always fault,
// the done signal is always 0x01;
static void axiJtagIp(void) {
  xlnx_axi_xvc->ptr->ctrl_offset = 0x01 << 1;
  usleep(1000);
  xlnx_axi_xvc->ptr->ctrl_offset = 0x00;
  LOG_INFO("reset axiJtageIp finish ctrl_offset=0x01<<1");
}
...
static int xlnx_axi_xvc_init(void) {
  ...
  // 							  MAP_SHARED, xlnx_axi_xvc->fd,
  // baseaddr);

#define MAP_SIZE 0x10000
  unsigned int pagesize = (unsigned)sysconf(_SC_PAGESIZE);
  xlnx_axi_xvc->ptr = (jtag_t *)mmap(
      NULL, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, xlnx_axi_xvc->fd,
      baseaddr & ~((typeof(baseaddr))pagesize - 1));
  if (xlnx_axi_xvc->baseaddr == MAP_FAILED) {
    LOG_ERROR("mmap() failed.  Check permissions.");
    close(xlnx_axi_xvc->fd);
    return ERROR_JTAG_INIT_FAILED;
  }
  LOG_INFO("Mapped Xilinx XVC/AXI vaddr %p paddr 0x%x", xlnx_axi_xvc->ptr,
           baseaddr);

  // LOG_INFO("Mapped Xilinx XVC/AXI vaddr %p paddr 0x%x",
...
static int xlnx_axi_xvc_speed(int speed) {
  // uint32_t *b = (uint32_t *)xlnx_axi_xvc->baseaddr + XLNX_XVC_DIV_REG;
  //  volatile uint32_t *w = (uint32_t *)b;
  ...
  // LOG_INFO("XVC/AXI Actual JTAG Clock Rate : %d Khz", (XLNX_XVC_BASE_FREQ /
  // divider) / 1000);
...
COMMAND_HANDLER(xlnx_axi_xvc_handle_devaddr_command) {
   ...
   */
  if (xlnx_axi_xvc->deviceaddr) free(xlnx_axi_xvc->deviceaddr);
...
COMMAND_HANDLER(xlnx_axi_xvc_handle_srst_command) {
   ...
   * before allocating a new one ...
   */
  if (xlnx_axi_xvc->srst.name) free(xlnx_axi_xvc->srst.name);
...
COMMAND_HANDLER(xlnx_axi_xvc_handle_trst_command) {
   ...
   * before allocating a new one ...
   */
  if (xlnx_axi_xvc->trst.name) free(xlnx_axi_xvc->trst.name);
...
static int xlnx_axi_xvc_swd_init(void) { return ERROR_OK; }
